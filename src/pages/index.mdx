...
A PDF book generator for [p5.js 2.0](https://p5js.org). No build tools. Just a `<script>` tag.
...
- [worksheet](/workshop/worksheet) — step-by-step exercises, no install required

## setup

_→ [hello book](/examples/01-hello-book) — createBook(), setDPI()_
...
You can pass an **options object** as the last argument:

```js
book = createBook(5, 8, 12, { imageType: "png" }); // lossless PNG
book = createBook(5, 8, 12, { jpegQuality: 0.98 }); // JPEG quality 0–1 (default 0.92)
book = createBook(5, 8, 12, { progressBar: true }); // full-screen overlay instead of corner counter
```
...
_→ [page numbers](/examples/02-page-numbers) · [color progression](/examples/03-color-progression) · [slow burn](/examples/06-slow-burn)_
...
**Note (LTR):** odd indices = left, even indices = right.
**Note (RTL):** odd indices = right, even indices = left.

## layout

_→ [spread layout](/examples/10-spread-layout) — setSpread(), isLeftPage(), isRightPage(), setSaddleStitch()_
...
_→ [bleed & print marks](/examples/05-bleed) — setBleed(), book.bleed.draw(), bleedWidth, bleedHeight_
...
_→ [spine & cover](/examples/11-spine-cover) — book.spine.draw(), saveCover(), setPageThickness(), set3DBackground()_
...
_→ [typography](/examples/04-typography) · [text flow](/examples/08-text-flow)_
...
_→ [text flow](/examples/08-text-flow) — book.finish(), textBox() overflow reflow_
...
_→ [spine & cover](/examples/11-spine-cover) — set3DBackground(), set3DEdgeColor()_
