...
- [Step-by-step workshop guide](./workshop/)
- [Docs index](./docs/)
- [Starter sketch (p5 editor)](https://editor.p5js.org/munusshih/sketches/u8Ox1CmnM)
- [Starter template (OpenProcessing)](https://openprocessing.org/@u261940/2975856)
...
### book.setSpread( viewSpread, [exportSpread] )

Set spread behavior for viewer and export in one call. The cover and back cover are kept as solo pages; inner pages are paired when spread is enabled. Total page count must be even when export spread is enabled. Call in `setup()` before `addPage()`.

```js
book.setSpread(true); // backward-compatible: view=true, export=true
book.setSpread(true, false); // spreads in viewer, single-page export
...
### book.setViewSpread( bool )

Control spread pairing in the viewer only.

```js
book.setViewSpread(true);
```

### book.setExportSpread( bool )

Control spread pairing in main PDF export and print only.

```js
book.setExportSpread(false); // export single pages
```
...
### book.setSkipCoversInMainPDF( bool )

Control whether the main PDF export includes cover + back cover.

- `false` (default): main PDF includes all pages.
- `true`: main PDF exports only interior pages (covers are still available in `saveCover()`).

Applies to both **Download -> PDF** and **Print** in the viewer.

```js
book.setSkipCoversInMainPDF(true); // main PDF = interior pages only
book.setSkipCoversInMainPDF(false); // include covers again
```

### book.setSeparateCoverStock( bool )

Enable perfect-binding cover workflow when the cover is printed as a separate sheet.

When enabled:

- `saveCover()` exports 2 pages:
  - page 1: back cover + spine + front cover
  - page 2: first interior + inner spine + last interior
- Main PDF (`Download -> PDF` and `Print`) excludes cover/back and first/last interior pages.

```js
book.setSeparateCoverStock(true);
```

Disable to return to the normal single-page cover export:

```js
book.setSeparateCoverStock(false);
```

Inside spine styling for page 2 (separate-cover-stock mode):
...
Override the computed spine width with an explicit value. Use this when your printer gives you an exact spine thickness and you want that value to be used directly.

```js
book.setSpineThickness(12); // 12 mm
book.setSpineThickness(0.47, "in"); // inches
book.setSpineThickness(null); // revert to auto-calculation
```
...
### book.setCropMarks( bool )

Show or hide crop marks (trim/bleed guides). `setBleed()` enables them automatically.

`book.setCropMark(bool)`, `book.setTrimMark(bool)`, and `book.setPrintMarks(bool)` are supported as aliases.
...
Download PDF with all pages (cover + interior).
...
- default: excludes front/back cover
- with `book.setSeparateCoverStock(true)`: also excludes first/last interior pages

### book.saveCoversOnly([filename])

Download a PDF with only cover pages.
