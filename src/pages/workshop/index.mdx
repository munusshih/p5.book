...
description: "A workshop for making books with code using p5.js and p5.book."
image: "/assets/book-cover-poster.png"
---

import Diagram from "@/components/Diagram.astro";

[← see the doc](../index.html)
...
<video
  ...
  muted
  playsinline
  poster="/assets/book-cover-poster.png"
>
...
<iframe
  src="https://editor.p5js.org/munusshih/full/oIt_CK3Zp"
  style="width: 100%; height: 500px; border: 1px solid #eee;"
></iframe>
...
- `background(255, 0, 0)` tells the background function to fill the canvas with red color (255 for red, 0 for green and blue).
- The `circle(100, 150, 50)` function call tells it to draw a circle at x=100, y=150 with a diameter of 50 pixels.
...
- Try the color picker here: <input type="color" id="colorPicker" value="#0000ff" />

- Or see a list of [color names here](https://www.w3schools.com/tags/ref_colornames.asp)
...
- So when you call `background("orange")`, it fills the canvas with a giant box of orange right away. When you call `circle(width / 2, height / 2, 50)`, it draws the circle on the canvas immediately.
- The order of your function calls matters, because they are executed in sequence. If you call `background()` after `circle()`, the background will cover the circle and you won't see it.
...
- `background()` can take a color name like "orange", or RGB values like `background(255, 165, 0)`
- While shape functions like `circle(x, y, diameter)` takes x and y coordinates and a diameter.
...
Hopefully you have some experience with p5.js already, but if not, no worries! We will try to explain and cover the basics as we go, and you can also check out the [p5.js reference](https://beta.p5js.org/reference/) for more details on how to use it.
...
Prefer OpenProcessing? Use this [starter template](https://openprocessing.org/@u261940/2975856).
...
```js
book.setSpread(true, true);
```

The two booleans mean:

- first `true` = spread layout in the **viewer**
- second `true` = spread layout in **PDF export/print**

Common combinations:

```js
book.setSpread(true, true); // spreads in viewer and export
book.setSpread(true, false); // spreads in viewer, single-page export (common print workflow)
book.setSpread(false, false); // no spreads
```
...
<div class="step" data-step="1">
  Try it out! Add `book.setSpread(true, true)` in `setup()`, and see how the
  canvas changes.
</div>
...
<div class="try">
  - What happens if you set `book.setSpread(true, true)` but don't draw anything
  that spans across the pages? Try it out and see how the left and right pages
  are still separate, but the canvas is wider. - Try making a spread where the
  left page has a shape that continues onto the right page. For example, a big
  circle that starts on the left and finishes on the right.
</div>
---
...
<div class="step" data-step="4">
  `setBleed()` turns crop marks on automatically, but you can toggle them.
...
function setup() {
  ...
  book.setCropMarks(false); // hide crop marks
  book.setCropMark(true); // singular alias, show them again
  book.setTrimMark(false); // alternate alias, hide them again
  // book.setCropMarks(true); // show them again
}
```
...
<div class="try">
  ...
  - Try turning crop marks off and on with `book.setCropMarks(false/true)`. -
...
- [Colllettivo.it](https://www.collletttivo.it/)
...
## <kbd>J</kbd> — custom spine

Beyond page content, you can also design the spine itself. p5.book exposes `book.spine`, a drawing surface that appears in the 3D preview and the cover PDF.

Use `setPageThickness()` when you want the spine width to be estimated from page count and paper thickness. Use `setSpineThickness()` when you already know the exact finished spine width from your printer specs.

```js
let book;

function setup() {
  book = createBook(5, 8, 120);
  book.setSpineThickness(12, "mm"); // explicit value takes priority
  book.setDPI(300);
}
...
function draw() {
  ...
  book.spine.draw((g) => {
    ...
    g.rotate(HALF_PI); // bottom -> top reading direction
...
Quick tips: If you use both methods, `setSpineThickness()` takes precedence over `setPageThickness()`.

---

## <kbd>K</kbd> — make your book
...
## <kbd>L</kbd> — ready for print

This part is really about exporting your files cleanly for print.

### Export flow

1. Use `book.setDPI(300)` in `setup()` for print-quality output.
2. Add bleed with `book.setBleed(0.125)` (US) or `book.setBleed(3, "mm")` (EU) if your printer needs it.
3. Keep `book.addPage()` as the last line in `draw()` so every page is captured correctly.
4. For most workflows: `book.setSpread(true, false)` (or `book.setViewSpread(true)` + `book.setExportSpread(false)`).
5. If covers should be delivered separately, call `book.setSkipCoversInMainPDF(true)` in `setup()`.
6. Export from the viewer and review at 100% zoom before sending to print.

For booklet printing/folding, enable Saddle Stitch before export:

```js
book.setSaddleStitch(true);
```

Then choose **Download -> Saddle Stitch** in the viewer.
...
For perfect binding (separate cover stock), use:

```js
book.setSeparateCoverStock(true);
```

This makes `Cover` export include a second side with `first interior | inner spine | last interior`, and makes the regular PDF export skip cover/back and also first/last interior pages.

```js
book.innerSpine = "#ffffff"; // blank/paper-like center panel
```

### Color note (important)

p5.book currently exports RGB PDFs. It does **not** support CMYK export yet.

That means:

- Bright neon RGB colors may print duller than on screen.
- Deep blacks may shift unless your printer applies proper conversion.
- You should ask your print shop for their preferred RGB-to-CMYK workflow/profile.

If your printer requires strict CMYK delivery, use p5.book for layout/content generation, then convert the exported PDF in a prepress tool before final print submission.

### Print preflight checklist

- Page count and order are correct (especially with spreads).
- Trim size matches your intended final book size.
- Bleed is present on all edges that need it.
- No critical text is too close to trim or spine.
- Cover, spine, and back cover align with your printer template.
- Fonts and thin lines remain readable at real print size.
- If using saddle stitch, page count is divisible by 4 and imposition order looks correct.

## <kbd>M</kbd> — cheat sheet
...
| `book.setSpread(true, true)`           | pair interior pages as spreads for viewer and export   |
...
| spread helpers fail (`isLeftPage`)              | spread mode not enabled                       | call `book.setSpread(true, true)` in `setup()`                    |
...
- Provide enough context about the library and your code, for instance include these links or the content from these links:
  - [p5.book library source](https://unpkg.com/p5.book@latest/p5.book.js)
...
```
I'm using p5.js 2.0 + a custom library called p5.book to create a book with code. Here's the link to the library source: https://unpkg.com/p5.book@latest/p5.book.js and the README: https://github.com/munusshih/p5.book/blob/main/README.md
...
| S      | advanced spine styling: textures, type systems, edge-safe rules | TBD    |
| T      | `saveSaddleStitch()` export and /4 page constraints             | TBD    |
| U      | advanced bleed tools: `bleed.draw()`, width, height             | TBD    |
| V      | crop mark control with `book.setCropMarks()`                    | TBD    |
